...
def train_model(train_data, test_data, output_dir, pretrained_model=None, epochs=100, batch_size=256, learning_rate=0.002, num_keypoints=133):
    # Device setup
    ...
    for epoch in range(epochs):
        ...
        if avg_test_loss < best_loss:
            ...
            best_epoch=epoch + 1
            torch.save(model.state_dict(), os.path.join(output_dir, f'net_{num_keypoints}_best.pth'))
            print(f'New best model saved (Epoch {best_epoch}, Loss: {best_loss:.6f})')

        # Modell speichern (jede 10. Epoche)
        if (epoch + 1) % 10 == 0:
            torch.save(model.state_dict(), os.path.join(output_dir, f'net_{num_keypoints}_epoch_{epoch + 1}.pth'))

    # Finales Modell speichern
    torch.save(model.state_dict(), os.path.join(output_dir, f'net_{num_keypoints}_final.pth'))
    print(f'\nTraining completed!')
    print(f'Best epoch: {best_epoch} with loss: {best_loss:.6f}')
    print(f'Final models saved in {output_dir}')

    return model


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Finetuning 2D to 3D Lifting Model")
    parser.add_argument('--input_files', nargs='+', required=True, help='Paths to input JSON files')
    parser.add_argument('--output_dir', type=str, required=True, help='Output directory for saved models')
    parser.add_argument('--pretrained_model', type=str, default=None, help='Path to pretrained model (optional)')
    parser.add_argument('--epochs', type=int, default=175, help='Number of epochs to train')
    ...
    for file_path in args.input_files:
        ...
        if isinstance(data, dict):
            train_data.extend(list(data.values()))
        else:
    ...
    test_loss=evaluate_model(model, test_data, device, num_keypoints=args.num_keypoints)
    print(f'Final Test Loss: {test_loss:.6f}')

    # Optional: Auch auf Trainingsdaten evaluieren
    train_loss=evaluate_model(model, train_data, device, num_keypoints=args.num_keypoints)
